...
class Pls(discord.Client):
    ...
    def __init__(self):
        ...
        file_handler = logging.FileHandler(filename=os.path.join(
            self._pls_dir, "pls.log"), encoding="utf-8", mode="w")
    ...
    def run(self, *args, **kwargs):
        ...
        try:
            self._twitter = Twython(api_keys.get(
                "twitter_app_key"), access_token=api_keys.get("twitter_access_token"))
            self._logger.info("plsbot sucessfully connected to Twitter.")
        except:
            self._logger.exception(
                "Could not connect to Twitter. Check your Twitter keyes in config.json.")
                        ...
                        if tweet and tweet.get("retweeted_status"):
                            ...
                            if tweet.get("extended_entities") and \
                               ...
                               tweet["extended_entities"]["media"][0]["type"] == "photo":
                                tweet_text = tweet_text.rsplit(
                                    " https://t.co/", 1)[0].strip()

                            if tweet_text != embed.get("description", "").strip():
                                await message.channel.send("Full tweet:\n```{}```".format(tweet["full_text"]))
                                self._logger.debug(
                                    "Sent untruncated version of tweet {}.".format(tweet_id))
    ...
    async def _event_on_message(self, message):
        ...
        for tweet_id in tweet_ids:
            self._logger.debug(
                "Saw a new message containing a tweet with id {}".format(tweet_id))
            tweet = await self._load_tweet(tweet_id)
            ...
            if tweet:
                ...
                if tweet.get("extended_entities") and tweet["extended_entities"].get("media"):
                    media_ids = [item["media_url_https"]
                                 for item in tweet["extended_entities"]["media"][1:] if item["type"] == "photo"]
                    if media_ids:
                        await message.channel.send("Additional images from tweet: {}".format(" ".join(media_ids)))
                        self._logger.debug(
